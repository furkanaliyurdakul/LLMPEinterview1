...
st.markdown("### 10. Assign a score from 1 to 5 to each of the following learning priorities based on their importance to you *")
...
st.markdown("### 14. What are your long-term academic or career goals related to this subject? *")
st.markdown("(Select one or more)")
...
st.markdown("### 15. What potential barriers do you anticipate encountering while studying this subject? *")
st.markdown("(Select one or more)")
...
for barrier in barriers:
    # Initialize checkbox in session state if not already present
    init_form_field(f"barrier_{barrier}", False)
    if st.checkbox(barrier, key=f"barrier_{barrier}", help="Select at least one option"):
...
if st.session_state.show_review:
    try:
        ...
        if 'form_data' not in st.session_state:
            st.error("Form data not found in session state. Please try submitting the form again.")
            st.session_state.show_review = False
        else:
